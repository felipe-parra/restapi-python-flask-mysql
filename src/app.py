...
def get_sales_by_user(id):
    ...
    """
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT u.name, SUM(s.amount) from sales as s, users as u where s.userId = u.id and u.id = %s group by u.id", (id))
    data = cursor.fetchall()
    print(data["name"])
    cursor.close()
    message = {
        "message": "sales by user",
        "data":data
...
def get_sales_by_teams(id):
    ...
    """
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT  t.name, SUM(s.amount) FROM sales AS s, users AS u, teams AS t WHERE s.userId = u.id AND u.teamId = t.id AND t.id = %s GROUP BY  t.id", (id))
    data = cursor.fetchall()
    cursor.close()
    message = {
        "message": "sales by user",
